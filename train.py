...
with tf.Session() as session:
    ...
    for step in range(100000):
        batch_x, batch_eye, batch_male, batch_young, batch_smiling = session.run(
            [tr_im_batch, tr_label_eye_batch, tr_label_male, tr_label_young, tr_label_smiling])

        _, loss_val, l2_loss_val, lr_val, global_step_val, loss_eyeglasses_val, \
        loss_male_val, loss_young_val, loss_smiling_val = \
            session.run([train_op, loss, l2_loss, lr, global_step, loss_eyeglasses,
                     loss_male, loss_young, loss_smiling], feed_dict={
            input_x:batch_x, label_eyeglasses: batch_eye,label_male:batch_male,
            label_young:batch_young, label_smiling:batch_smiling})


        print("ite:{}, loss:{}, l2_loss:{}, loss_eye:{}, loss_male:{}, "
              "loss_young: {}, loss_smiling: {}, lr:{}".format(step, loss_val, l2_loss_val, loss_eyeglasses_val,
                                                    loss_male_val, loss_young_val, loss_smiling_val, lr_val))
        ...
        summary_writer.add_summary(tf.Summary(
            value = [tf.Summary.Value(tag = "loss_smiling", simple_value = loss_smiling_val)]),
            global_step=step
        )
        ...
        if step % 500 == 0:
            batch_x, batch_eye, batch_male, batch_young, batch_smiling = session.run(
                [te_im_batch, te_label_eye_batch, te_label_male, te_label_young, te_label_smiling])

            loss_val, l2_loss_val, loss_eyeglasses_val, \
            loss_male_val, loss_young_val, loss_smiling_val = \
                session.run([loss, l2_loss, loss_eyeglasses,
                             loss_male, loss_young, loss_smiling], feed_dict={
                    input_x: batch_x, label_eyeglasses: batch_eye, label_male: batch_male,
                    label_young: batch_young, label_smiling: batch_smiling})

            print("Test, ite:{}, loss:{}, l2_loss:{}, loss_eye:{}, loss_male:{}, "
                  "loss_young: {}, loss_smiling: {}".format(step, loss_val, l2_loss_val, loss_eyeglasses_val,
                                                                   loss_male_val, loss_young_val, loss_smiling_val
                                                                   ))
